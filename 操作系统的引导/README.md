...
x86 PC机刚开机时，CPU 处于 8086 实模式，cs、ip 寄存器被分别初始化为 *0xFFFF*，*0x0000*，为 ROM BIOS 映射区。该程序检查 RAM、键盘、显示器、软硬盘及其他设备，随后将磁盘的 0 磁道 1扇区（操作系统引导扇区）读入内存 *0x7C00* <sup>[1]</sup>处。



## 2	实验内容

### 2.1	改写 Linux 引导程序 bootsect.s
